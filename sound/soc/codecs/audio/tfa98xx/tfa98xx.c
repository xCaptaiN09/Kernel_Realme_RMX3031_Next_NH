...
#if defined(OPLUS_BUG_COMPATIBILITY)
...
#else //OPLUS_BUG_COMPATIBILITY && CONFIG_TFA9874_NONDSP_STEREO
...
#endif //OPLUS_BUG_COMPATIBILITY && CONFIG_TFA9874_NONDSP_STEREO
					      ...
					      bool unregister)
{
	...
	for (i = 0; i < tfa_cnt_get_dev_nprof(tfa98xx->tfa); i++) {
		#ifdef OPLUS_ARCH_EXTENDS
		char* profile_name = tfa_cont_profile_name(tfa98xx, i);
		if ((profile_name != NULL) && strstr(profile_name, ".tap")) {
			tap_profile = true;
				...
				int num_dai)
{
	...
	int i;
	#ifdef OPLUS_ARCH_EXTENDS
	unsigned int i2cbus = i2c->adapter->nr;
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	if (strcmp(dflt_prof_name, "")) {
		...
		for (i = 0; i < nprof; i++) {
			#ifdef OPLUS_ARCH_EXTENDS
			char* profile_name = tfa_cont_profile_name(tfa98xx, i);
			if ((NULL != profile_name) && (strcmp(profile_name,
							dflt_prof_name) == 0)) {
				tfa98xx->profile = i;

...
static int do_get_info(void __user *arg)
{
	...
	cmd.flags |= 0x8; // Next
	cmd.flags |= 0x10; // Legacy
	cmd.flags |= 0x20; // Legacy
	cmd.flags |= 0x40; // Hide GKI1

	cmd.features = 7; // SU_COMPAT | KERNEL_UMOUNT | SUSFS

	get_task_comm(comm, current);
	if (ksuver_override) {
		cmd.version = ksuver_override;
	} else if (strstr(comm, "ksud") && !strstr(comm, "libksud")) {
		cmd.version = 12882; // Satisfy ReZygisk check (< 20000) for CLI
	} else {
		cmd.version = 32945; // Satisfy Manager check (>= 30000)
	}
...
static int do_get_id(void __user *arg)
{
	...
	strscpy(cmd.id, "KernelSU Next", sizeof(cmd.id));
...
static const struct ksu_ioctl_cmd_map ksu_ioctl_handlers[] = {
	...
	{ .cmd = KSU_IOCTL_NUKE_EXT4_SYSFS, .name = "NUKE_EXT4_SYSFS", .handler = do_nuke_ext4_sysfs, .perm_check = manager_or_root },
	    ...
	    { .cmd = KSU_IOCTL_GET_VERSION_TAG,
	      ...
	      .handler = do_get_hook_mode, // Swapped for Legacy Protocol
	      .perm_check = always_allow },
	    { .cmd = KSU_IOCTL_GET_HOOK_MODE,
	      .name = "GET_HOOK_MODE",
	      .handler = do_get_version_tag, // Swapped for Legacy Protocol
	      .perm_check = always_allow },
	    { .cmd = _IOC(_IOC_READ, 'K', 21, 0), // GET_ID
	      .name = "GET_ID",
	      .handler = do_get_id,
	      .perm_check = always_allow },
...
void ksu_supercalls_init(void)
{
    ...
    ksu_susfs_init(); // Initialize SUSFS features and register handler
